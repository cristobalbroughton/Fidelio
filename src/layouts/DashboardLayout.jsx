...
function PlanBanner({ status, className = '' }) {
  ...
  return (
    <a
      ...
      role={isGrace || atLimit ? 'alert' : undefined}
      className={`flex items-start gap-2 px-3 py-2.5 rounded-lg border text-xs leading-snug hover:opacity-80 transition-opacity ${bg} ${textCls} ${className}`}
    >
      <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-px" strokeWidth={2} aria-hidden />
      <span>{msg} <span aria-hidden>→</span></span>
    </a>
  )
}
...
// Mobile bottom nav (máx. 6 ítems — Ayuda vive en el header móvil)
const BOTTOM_NAV = [
  { to: '/dashboard',                    icon: LayoutDashboard, label: 'Inicio',    end: true  },
...
function DashboardLayoutInner() {
  ...
  const handleAnalyticsLocked = () => {
    toast('Analytics avanzado es exclusivo del plan Pro. Actualiza tu plan para ver tus métricas completas.', {
      icon: <Lock className="w-4 h-4 text-primary" aria-hidden />,
      duration: 4000,
    })
  }
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <header className="md:hidden fixed top-0 inset-x-0 h-14 z-40 bg-dark border-b border-white/[0.06] flex items-center justify-between px-4">
        ...
        {/* Business name + ayuda + logout */}
        <div className="flex items-center gap-1">
          <span className="text-[12px] font-medium truncate max-w-[110px] text-white/55">{businessName}</span>
          <Link
            to="/ayuda"
            aria-label="Ayuda"
            className="flex items-center justify-center w-11 h-11 text-white/55 hover:text-white/85 transition-colors duration-150"
          >
            <HelpCircle className="w-[17px] h-[17px]" strokeWidth={1.8} aria-hidden />
          </Link>
          <button
            onClick={handleLogout}
            disabled={loggingOut}
            aria-label="Cerrar sesión"
            className="flex items-center justify-center w-11 h-11 text-white/55 hover:text-red-400 transition-colors duration-150 disabled:opacity-50"
          >
            <LogOut className="w-[17px] h-[17px] shrink-0" strokeWidth={1.8} aria-hidden />
          </button>
        </div>
            ...
            if (proOnly && !isPro) {
              return (
                <button
                  ...
                  aria-label={`${label} (requiere plan Pro)`}
                  className="group relative flex items-center gap-3 px-3 rounded-lg text-[13.5px] font-medium transition-all duration-150 select-none min-h-[42px] w-full text-left text-white/40 hover:bg-white/[0.04] cursor-pointer"
                >
                  <Icon className="w-[17px] h-[17px] shrink-0 text-white/30" strokeWidth={1.8} aria-hidden />
                  <span className="flex-1">{label}</span>
                  <Lock className="w-[13px] h-[13px] text-white/30 shrink-0" strokeWidth={1.8} aria-hidden />
                </button>
              )
            }
            return (
        ...
        <div className="px-2.5 pb-5 pt-3 space-y-0.5">
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 min-h-[42px]">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-white/45 uppercase tracking-[0.1em] font-medium leading-none mb-1">
                Negocio
              </p>
          ...
          <button
            onClick={handleLogout}
            disabled={loggingOut}
            className="group w-full flex items-center gap-3 px-3 rounded-lg text-[13.5px] font-medium text-white/45 hover:text-red-400 hover:bg-red-500/[0.07] transition-all duration-150 min-h-[42px] cursor-pointer disabled:opacity-50"
          >
            <LogOut
              className="w-[17px] h-[17px] shrink-0 text-white/35 group-hover:text-red-400 transition-colors duration-150"
              strokeWidth={1.8}
              aria-hidden
            />
            ...
            if (proOnly && !isPro) {
              return (
                <button
                  ...
                  aria-label={`${label} (requiere plan Pro)`}
                >
                  <div className="flex flex-col items-center justify-center gap-1 py-3 min-h-[48px] relative text-white/35">
                    <Icon className="w-5 h-5 shrink-0" strokeWidth={1.7} aria-hidden />
                    <span className="text-[10px] font-medium leading-none tracking-wide flex items-center gap-0.5">
                      {label}
                      <Lock className="w-[9px] h-[9px] shrink-0" strokeWidth={2} aria-hidden />
                    </span>
                  </div>
                </button>
              )
            ...
            return (
              <NavLink key={to} to={to} end={end} className="flex-1">
                {({ isActive }) => (
                  <div
                    className={[
                      'flex flex-col items-center justify-center gap-1 py-3 min-h-[48px] transition-colors duration-150 relative',
      ...
      <div className="flex min-h-screen flex-col">
        <header className="fixed top-0 inset-x-0 h-14 z-40 bg-dark border-b border-white/[0.06] flex items-center justify-between px-4">
          ...
          <button
            ...
            aria-label="Cerrar sesión de cajero"
            className="flex items-center gap-1.5 text-white/55 hover:text-red-400 transition-colors duration-150 min-h-[44px] py-1 pl-2"
          >
            <span className="text-[12px] font-medium truncate max-w-[100px]">{cashier.cashier_name}</span>
            <LogOut className="w-[15px] h-[15px] shrink-0" strokeWidth={1.8} aria-hidden />
          </button>
        </header>
